function(_tao_path_list path_list_var)
...
function(_tao_append_runtime_lib_dir_to_path dst)
  if(MSVC)
  ...
  _tao_path_list(path_list ${path_list})
  if(NOT MSVC)
...
function(_tao_get_generated_output_dir target output_dir_var)
  ...
  set(output_dir "${CMAKE_CURRENT_BINARY_DIR}/tao_generated/${target}")
...
function(_tao_get_generated_output target file)
  ...
  _tao_get_generated_output_dir("${target}" output_dir ${dir_args})
...
function(_tao_idl target)
  set(no_value_args BUILD_SEPARATE)
  set(one_value_args AUTO_INCLUDES INCLUDE_BASE)
  set(multi_value_args IDL_FLAGS IDL_FILES)
  cmake_parse_arguments(arg "${no_value_args}" "${one_value_args}" "${multi_value_args}" ${ARGN})

  if(NOT arg_IDL_FILES)
    message(FATAL_ERROR "called _tao_idl(${target}) without specifying IDL_FILES")
  endif()
  ...
  _tao_get_generated_output_dir(${target} auto_includes O_OPT "${idl_cmd_arg_-o}" MKDIR)
  if(arg_INCLUDE_BASE)
  ...
  foreach(idl_file ${arg_IDL_FILES})
    ...
    _tao_get_generated_output(${target} "${idl_file}"
    ...
    _tao_append_runtime_lib_dir_to_path(extra_lib_dirs)
    ...
    if(arg_BUILD_SEPARATE)
      set_source_files_properties(${cpp_files} PROPERTIES SKIP_UNITY_BUILD_INCLUSION TRUE)
    endif()
    target_sources(${target} PRIVATE ${cpp_files})
  endforeach()

  target_include_directories(${target} PUBLIC ${auto_includes})
